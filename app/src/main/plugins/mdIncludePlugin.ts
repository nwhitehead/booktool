...
Forked from: https://github.com/camelaissani/markdown-it-include

Main change here is to add support for "@/path/filename.md" type paths. The "@"
symbol indicates an internal library root directory different than the main
root.
...
export default (md, options) => {
        ...
        while ((cap = options.includeRe.exec(src))) {
            ...
            if (!errorMessage) {
                if (includePath.startsWith('@/')) {
                    filePath = path.resolve(options.atRoot, `./${includePath.slice(2)}`);
                } else {
                    filePath = path.resolve(options.root, includePath);
                }
    ...
    const _includeFileParts = (state, startLine, endLine/*, silent*/) => {
        state.src = _replaceIncludeByContent(state.src, options.getRootDir(options, state, startLine, endLine), undefined, undefined);
    };
